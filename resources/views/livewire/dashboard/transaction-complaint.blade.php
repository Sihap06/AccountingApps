                    ...
                    @foreach ($data as $index => $item)
                        <tr wire:key='{{ $index . time() }}' wire:loading.remove
                            ...
                            <td
                                class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                <span
                                    class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400 uppercase">
                                    {{ $item->customer_name }}
                                </span>
                            </td>
                            <td
                            ...
                            <td
                                ...
                                <div class="flex flex-row gap-x-3">
                                    ...
                                    <button wire:click="$emit('triggerCompleteTransaction',{{ $item->id }})"
                                        class="inline-flex gap-x-2 items-center px-3 py-2 text-xs mr-3 font-bold text-center text-white uppercase align-middle transition-all rounded-lg cursor-pointer bg-green-500 leading-normal  ease-in tracking-tight-rem shadow-md bg-150 bg-x-25 hover:-translate-y-px active:opacity-85 hover:shadow-md">
                                        <div wire:loading wire:target='TransactionComplete("{{ $item->id }}")'>
                                            <div class="inline-block h-3 w-3 animate-spin rounded-full border-2 border-solid border-current border-r-transparent align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite]"
                                                role="status">
                                                <span
                                                    class="!absolute !-m-px !h-px !w-px !overflow-hidden !whitespace-nowrap !border-0 !p-0 ![clip:rect(0,0,0,0)]">Loading...</span>
...
@push('script')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @this.on('triggerCompleteTransaction', id => {
                Swal.fire({
                    title: 'Make sure the transaction is complete',
                    html: "",
                    icon: 'warning',
